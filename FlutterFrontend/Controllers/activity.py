...
def history(req):
    ...
    if(req.method == "GET"):
        history = ReadingHistory.objects.filter(user_id=user).select_related('article')\
  ...
  #localhost:8000/history/?article=13
        ...
        if article_id:
            ...
            history = list(ReadingHistory.objects.filter(user_id=user, article_id=article_id))
            if history != []:
               history = ReadingHistory.objects.get(user_id=user, article_id=article_id)
               history.history_date = datetime.now()
              
            else:
                history = ReadingHistory(user_id=user, article_id=article_id)
...
def bookmark(req):
    ...
    if(req.method == "GET"):
        bookmark = Bookmark.objects.filter(user_id=user).select_related('article').annotate(id=F('article__article_id')\
        ...
        if article_id:
            ...
            bookmark = Bookmark(user_id=user, article=article)
            bookmark.save()
    ...
    #localhost:8000/bookmark/?article_id=1
        ...
        if article_id:
            article_id = int(article_id)
            article = Article.objects.get(article_id = article_id)
            bookmark = Bookmark.objects.filter(article=article, user_id=user)
            bookmark.delete()
...
def bookmark_article_ids(req):
    ...
    user = authenticate(req)
    article_ids = list(Bookmark.objects.filter(user_id=user).select_related('article').\
...
def like_article(req):
    ...
    if(req.method == "GET"):
        likes = Likes.objects.filter(user_id=user).select_related('article').annotate(id=F('article__article_id')\
            ,title=F('article__title'),link=F('article__link'),summary=F('article__summary')\
                    ,description=F('article__description'),image_url=F('article__image_url'),\
                    category=F('article__category'),source=F('article__source'), publication_date=F('article__publication_date'), date=F('article__date')\
        ...
        if article_id:
            article_id = int(article_id)
            article = Article.objects.get(article_id = article_id)
            likes = Likes(user_id=user, article=article)
            likes.save()
    ...
    #localhost:8000/like/?article_id=1
        ...
        if article_id:
            article_id = int(article_id)
            article = Article.objects.get(article_id = article_id)
            likes = Likes.objects.filter(article=article, user_id=user)
            likes.delete()
...
def like_article_ids(req):
    # localhost:8000/bookmark_article_ids
    user = authenticate(req)
    article_ids = list(Likes.objects.filter(user_id=user).select_related('article').\
        annotate(id=F('article__article_id')).values_list("id", flat=True))
...
def not_interested(req):
	...
	if article_id:
		 article_id = int(article_id)
		 article = Article.objects.get(article_id = article_id)	
		 not_interested = NotInterested(user_id=user, article=article)
		 not_interested.save()
